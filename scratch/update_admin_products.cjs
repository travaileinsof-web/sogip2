...
const componentStart = /const AdminProducts: React\.FC = \(\) => {/;
const stateInsert = `
...
`;
content = content.replace(componentStart, 'const AdminProducts: React.FC = () => {' + stateInsert);
...
fs.writeFileSync('src/pages/admin/AdminProducts.tsx', content, 'utf8');
